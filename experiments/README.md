...
| [experiment-uni2-camelyon16-abmil.py](experiment-uni2-camelyon16-abmil.py) | experiment | Trains `ABMILite` on a directory of slide embeddings (`--embeds-dir`, defaults to `CAMELYON16_EMBEDS_DIR`) with one stratified train/val split, evaluates on the held-out `test_*` slides, then runs instance-level inference to build the logit/attention "ZA space" and compares attention, logit, and a Bayesian Gaussian Mixture cluster score as tumor-patch predictors. Saves the trained checkpoint (`abmilite_camelyon16_{run-name}.pt`, tag defaulting to the `--embeds-dir` folder name) and figures to `OUTPUTS_DIR/camelyon16/`. Takes `--embeds-dir`, `--embedding-dim` (default 1536, for UNI2-H), `--run-name`, and `--feature-fraction` (approach 2: fixed random subset of embedding dims, see `experiment-feature-fraction-sweep.py` below for the full replicate sweep over this). Run this before the two scripts below. |
| [experiment-feature-fraction-sweep.py](experiment-feature-fraction-sweep.py) | experiment | Sweeps the script above's approach-2 (fixed random feature subset) across `--fractions` (default `1.0 0.5 0.25 0.125`) with a nested `--feature-versions` x `--model-versions` replicate design (5x5 by default, forced to `feature-versions=1` at fraction=1.0), mirroring the Proportion MNIST replicate methodology. Shared single-trial train/eval logic lives in `src/camelyon16_training.py`. Resumable — skips any `(fraction, feature_version, model_version, epochs, lr)` combination already present in the output CSV. Writes one row per replicate to `feature_fraction_sweep.csv` (all bag/instance/correlation metrics needed by the figures script below), plus a representative za-space scatter per fraction (`--za-plot-feature-version`/`--za-plot-model-version`, default the first replicate) using the same filename convention as the script above. |
| [figures-feature-fraction-sweep.py](figures-feature-fraction-sweep.py) | figures | Reads `feature_fraction_sweep.csv` and renders 3 figures (instance ROC AUC, logit-attention correlation, bag performance — each vs. feature fraction, mean +/- std across replicates) styled to match Fig 1D in the manuscript, to `OUTPUTS_DIR/camelyon16/figures/`. Takes `--csv` and `--out-dir`. |
| [experiment-camelyon-uncertainty.py](experiment-camelyon-uncertainty.py) | experiment | Loads the checkpoint from `experiment-uni2-camelyon16-abmil.py` and bootstraps (slide-level resampling) the patch-level ROC-AUC of attention, instance logit, and GMM cluster score to get confidence intervals, plus a repeated-measures ANOVA + Tukey HSD post-hoc test across the three methods. Writes `bootstrap-CI-camelyon16.csv` and a violin/box comparison figure. |
