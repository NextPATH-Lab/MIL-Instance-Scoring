...
| [utils-zarrify-wsi.py](utils-zarrify-wsi.py) | CLI utility | Slices each WSI's tissue ROI into overlapping tiles, splits them into tumor/benign using the ASAP XML tumor annotation (if present), and writes them plus their level-0 coordinates into a Zarr v3 store per slide. Takes `-d/--directory`, `-i/--indices`, `-s/--size`, `-o/--overlap-size` args. |
